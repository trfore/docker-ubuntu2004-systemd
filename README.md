...
[![CD](https://github.com/trfore/docker-ubuntu2004-systemd/actions/workflows/cd.yml/badge.svg)](https://github.com/trfore/docker-ubuntu2004-systemd/actions/workflows/cd.yml)
...
- Install [docker]
- Build an image locally (see above) or pull from Docker Hub: `docker pull trfore/docker-ubuntu2004-systemd:latest`
- Run a container from the image:

  ```sh
  docker run -d -it --name ubuntu2004-systemd --privileged --cgroupns=host --tmpfs=/run --tmpfs=/tmp --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro trfore/docker-ubuntu2004-systemd:latest
  ```

- Use it, example:

  ```sh
  docker exec -it ubuntu2004-systemd /bin/bash
  ```
...
- Podman defaults to running containers in systemd mode, `--systemd=true`, and will mount the required tmpfs and cgroup
  ...
  ```sh
  podman run -d -it --name ubuntu2004-systemd docker.io/trfore/docker-ubuntu2004-systemd:latest
  ```
